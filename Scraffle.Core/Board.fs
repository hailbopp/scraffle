...
module ScoreMultiplier =
    ...
    let getSpaceMultiplier size x y =
        ...
        | _ ->
            ...
            let isCentral = 0.0 < distance && distance <= 2.0
            let isCorner = abs cx = max && abs cy = max
            
            multipliers 
                |> List.sortByDescending
                    (match distance with
                        | d when (d / (float max)) < 0.5 -> (fun m -> m.scope = Letter, m.coefficient * -1.0)
                        | _ -> (fun m -> (m.scope = Word, m.coefficient)))
                |> List.map Some
                |> List.head
